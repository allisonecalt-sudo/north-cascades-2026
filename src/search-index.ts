...
function indexViewpoints(): SearchEntry[] {
  ...
  for (const v of VIEWPOINT_DESTINATIONS) {
    ...
    out.push({
      ...
      deepLinkUrl: `things-to-do.html#${v.id}`,
      tags: [
        v.corridor,
        v.effort,
        v.bestTime,
  ...
  for (const v of VIEWPOINTS) {
    ...
    out.push({
      ...
      deepLinkUrl: `things-to-do.html#mp-${v.milepost}`,
  ...
  for (const v of BAKER_VIEWPOINTS) {
    ...
    out.push({
      ...
      deepLinkUrl: `things-to-do.html#baker-${slug(v.name)}`,
...
function indexLakes(): SearchEntry[] {
  return LAKES.map((l): SearchEntry => {
    ...
    return {
      ...
      region: l.where,
      deepLinkUrl: `things-to-do.html#${l.id}`,
      tags,
...
function indexTowns(): SearchEntry[] {
  return TOWNS.map((t): SearchEntry => {
    ...
    return {
      ...
      deepLinkUrl: `things-to-do.html#towns`,
      tags,
...
function indexActivities(): SearchEntry[] {
  return ACTIVITIES.map((a): SearchEntry => {
    ...
    return {
      ...
      region: a.where,
      deepLinkUrl: `things-to-do.html#${a.id}`,
      tags,
...
function indexSunsets(): SearchEntry[] {
  return TOP_SUNSETS.map((s): SearchEntry => {
    ...
    return {
      ...
      region: s.where,
      deepLinkUrl: `things-to-do.html#sunset-${s.rank}`,
      tags,
      weight,
    };
  });
...
const PAGE_ROWS: PageRow[] = [
  ...
  { id: 'things-to-do', name: 'Things to Do', url: 'things-to-do.html', blurb: 'Activities, lakes & water, viewpoints, and sunsets — all in one place.', tags: ['things-to-do', 'activities', 'lakes', 'viewpoints', 'sunsets'], weight: 80 },
  { id: 'viewpoints', name: 'Viewpoints', url: 'things-to-do.html#viewpoints', blurb: 'Drive-up postcards along WA-20 + Mt. Baker corridor.', tags: ['viewpoints', 'overlooks', 'drive-up'], weight: 78 },
  ...
  { id: 'towns', name: 'Towns', url: 'things-to-do.html#towns', blurb: 'Marblemount → Newhalem → Concrete → Mazama → Winthrop.', tags: ['towns', 'corridor'], weight: 68 },
  ...
  { id: 'for-erin', name: 'For Erin', url: 'things-to-do.html', blurb: 'Open decisions to weigh in on.', tags: ['erin', 'decisions', 'feedback'], weight: 75 },
